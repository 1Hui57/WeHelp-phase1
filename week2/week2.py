...
def find_and_print(messages, current_station):
    ...
    QizhangIndex=greenLine.index('Qizhang')
    #計算如果是在小碧潭站，每個站與小碧潭站的距離
    distanceOfCurrentStation={}
    if current_station=='Xiaobitan':
        for station in greenLine:
            distanceOfCurrentStation[station]=abs(QizhangIndex-greenLine.index(station))+1
        distanceOfCurrentStation['Xiaobitan']=0
    #計算如果不是在小碧潭站，每個站與currentStation的距離
    else:
        currentStationIndex=greenLine.index(current_station)
        for station in greenLine:
            distanceOfCurrentStation[station]=abs(currentStationIndex-greenLine.index(station))
        distanceOfCurrentStation['Xiaobitan']=abs(QizhangIndex-greenLine.index(current_station))+1
    #以小排到大
    distanceOfCurrentStation=sorted(distanceOfCurrentStation.items(), key=lambda x:x[1])
    listdistanceOfCurrentStation=list(distanceOfCurrentStation)
    #如果列表中的第一個站明有出現在某人說過的話，印出那個人名
    for name, say in messages.items():
        if listdistanceOfCurrentStation[0][0] in say:
            print(name)
            return
    #計算最近的人
    for station in listdistanceOfCurrentStation:
        for name, say in messages.items():
            if station[0] in say:
                print(name)
                return
